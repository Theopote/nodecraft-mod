package com.nodecraft.gui.editor.util;
    ...
    public void close() {
        ...
        if (fontPushed) {
            try {
                ImGui.popFont();
                fontPushed = false;
                NodeCraft.LOGGER.debug("成功弹出字体");
            } catch (Exception e) {
                NodeCraft.LOGGER.error("弹出字体时出错: {}", e.getMessage());
                fontPushed = false; // 重置标志
            }
        }
        
        // 最终检查ImGui栈状态，尝试进一步修复可能的栈不平衡问题
        try {
            // 这里添加额外的ImGui状态检查代码
            // 由于ImGui Java绑定可能没有直接访问栈状态的方法，这部分可能需要根据实际情况调整
            NodeCraft.LOGGER.debug("ImGuiStyleScope关闭完成");
        } catch (Exception e) {
            NodeCraft.LOGGER.error("检查ImGui状态时出错: {}", e.getMessage());
        }
    }
} 