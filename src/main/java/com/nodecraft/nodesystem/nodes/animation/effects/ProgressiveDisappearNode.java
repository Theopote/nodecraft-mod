...
/**
 * Progressive Disappear Node: 逐步消失节点
 * 根据时间因子逐个或逐组地隐藏几何体
 */
@NodeInfo(
    id = "animation.effects.progressive_disappear",
    displayName = "Progressive Disappear",
    description = "随时间逐步隐藏几何体",
    category = "animation.effects"
)
public class ProgressiveDisappearNode extends BaseNode {

    // --- 输入端口 IDs ---
    private static final String INPUT_GEOMETRY_ID = "input_geometry";
    private static final String INPUT_TIME_FACTOR_ID = "input_time_factor";
    private static final String INPUT_DISAPPEAR_ORDER_ID = "input_disappear_order";
    ...
    // --- 消失顺序枚举 ---
    public enum DisappearOrder {
        RANDOM(0, "Random", "随机顺序"),
        BY_HEIGHT(1, "By Height", "按高度从上到下"),
        ...
        DisappearOrder(int id, String name, String description) {
            this.id = id;
            this.name = name;
            this.description = description;
        }
        
        public static DisappearOrder fromId(int id) {
            for (DisappearOrder order : values()) {
                if (order.id == id) {
                    return order;
                }
            }
    ...
    // --- 构造函数 ---
    public ProgressiveDisappearNode() {
        super(UUID.randomUUID(), "animation.effects.progressive_disappear");
        ...
        addInputPort(new BasePort(INPUT_DISAPPEAR_ORDER_ID, "Disappear Order", "消失顺序（0=随机, 1=按高度, 2=按距离, 3=按索引）", NodeDataType.INTEGER, this));
        addInputPort(new BasePort(INPUT_GROUP_SIZE_ID, "Group Size", "每次隐藏的方块数", NodeDataType.INTEGER, this));
    ...
    public void processNode(@Nullable ExecutionContext context) {
        ...
        Integer disappearOrderId = (Integer) inputValues.getOrDefault(INPUT_DISAPPEAR_ORDER_ID, DisappearOrder.SEQUENTIAL.id);
        ...
        // 获取消失顺序
        DisappearOrder disappearOrder = DisappearOrder.fromId(disappearOrderId);
        
        // 处理几何体
        List<Object> visibleGeometry = processGeometry(geometryObj, timeFactor, disappearOrder, groupSize, referencePoint);
     ...
     */
    private List<Object> processGeometry(Object geometryObj, float timeFactor, DisappearOrder disappearOrder, int groupSize, float[] referencePoint) {
        ...
        if (geometryObj instanceof List) {
            ...
            // 根据消失顺序对几何体排序
            List<Object> sortedGeometry = sortGeometry(geometryList, disappearOrder, referencePoint);
            
            // 计算要隐藏的方块数量（根据时间因子）
            int totalBlocks = sortedGeometry.size();
            int blocksToHide = Math.min(totalBlocks, Math.round(totalBlocks * timeFactor));
            
            // 确保blocksToHide是groupSize的倍数
            blocksToHide = (blocksToHide / groupSize) * groupSize;
            
            // 添加可见的方块到结果（隐藏的在前面，所以从隐藏数量开始添加）
            for (int i = blocksToHide; i < totalBlocks; i++) {
     ...
     * 根据消失顺序对几何体排序
     */
    private List<Object> sortGeometry(List<Object> geometry, DisappearOrder disappearOrder, float[] referencePoint) {
        List<Object> sortedGeometry = new ArrayList<>(geometry);
        
        switch (disappearOrder) {
            case RANDOM:
                // 随机排序
